...
export async function POST(req: Request) {
    try {
        ...
        console.log(search_name,startdate)
        const response = await fetch(`${process.env.FRAPPE_URL}/api/method/matsapp.api.event.event.get_post_expense_list?search_name=${search_name}&startdate=${startdate}&enddate=${enddate}&page_no=${page_no}&status=${status}&requestor=${requestor}`, {
            method: 'GET',
            headers: {
