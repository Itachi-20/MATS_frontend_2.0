...
const Form3 = ({ ...Props }: Props) => {
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    setTimeout(() => {
      router.push(`/monetary_grant?forms=4&refno=${refNo}`)
    }, 500)
  };
  ...
  const handleNavigation = () => {
    const fromValue = encodeURIComponent(`monetary_grant?forms=3&refno=${refNo}`);
  ...
  return (
    ...
    (<div>
          ...
          <TableBody>
            {previewData && previewData.compensation.map((item, index) => (
              ...
              <TableRow className="text-black text-center">
                ...
                <TableCell>
                  <div className="flex justify-around">
                    <div className="hover:cursor-pointer" onClick={() =>{setDeleteName(item.name);setDeleteType("Compansation");setIsDeleteDialog(prev=>!prev)}}>
                      <Image src={"/svg/delete.svg"} width={20} height={20} alt='view-document' className='cursor-pointer' />
                    </div>
                  </div>
                </TableCell>
      ...
      <div className="flex justify-end pt-5 gap-4">
        ...
        <Button className="bg-white text-black border text-md font-normal hover:bg-white" onClick={() => router.push(`/monetary_grant?forms=2&refno=${Props.refno}`)}>
          Back
        </Button>
